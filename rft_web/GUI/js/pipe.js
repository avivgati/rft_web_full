PipeDraw = function(msg) {
    var that = this,
        graph, parent, layout, nodesCount = 0,
        nodes = {}, links = {}, loadDone = false, depthLevel = 0;
    ...
    var container = msg.name, outlineContainer = container+"Outline", content, tabTitle = container;
    ...
    content += '        <fieldset class="bordered" style="width:200px;float:left;">';
	content += '            <legend>Orientation</legend>';
	content += '            <div class="pretty p-default p-curve" style="margin-bottom:10px;">';
	content += '                <input type="radio" name="viewOrientation'+container+'" value="Vertical" checked="checked"/>';
    content += '                <div class="state p-primary-o">';
    content += '			    	<label>Vertical</label>';
    content += '			    </div>';
	content += '			</div><br>';
    content += '			<div class="pretty p-default p-curve">';
    content += '				<input type="radio" name="viewOrientation'+container+'" value="Horizontal"/>';
    content += '				<div class="state p-success-o">';
    content += '				    <label>Horizontal</label>';
    content += '			    </div>';
	content += '		    </div>';
    content += '		</fieldset>';
    content += '        <fieldset class="bordered" style="width:200px;float:left;">';
    content += '            <legend>Depth</legend>';
    content += '            <input type="range" min="1" max="100" value="50" class="slider" id="myRange">';
    content += '		</fieldset>';
    content += '    </div>';
    ...
    that.scrollToMiddle = function()
    {
        var ranks = layout.model.ranks;
        var middle = Math.floor(ranks.length/2);
        var row = ranks[middle];
        middle = Math.floor(row.length/2);
        var cell = row[middle].cell;
        graph.fit(50);
        graph.scrollCellToVisible(cell, true);
    }
    that.hideToLevel = function(level)
    {
        if (level == 0)
        {
        ...
        else
        {
            ...
            {
                var rank = ranks[i], visible = i < level;
    ...
    $("#myRange").on('change', function() {
        depthLevel = this.value;
        that.hideToLevel(depthLevel);
    });
    // Add change event to the orientation radio buttons
    $('input[type=radio][name=viewOrientation'+container+']').change(function() {
        ...
        that.executeLayout();
        that.scrollToMiddle();
        //graph.fit();//50,null,150,null,null,null,0.9);
        //graph.center();
    });
    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

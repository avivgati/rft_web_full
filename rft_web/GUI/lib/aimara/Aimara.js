...
function createTree(p_div,p_backColor,p_contextMenu) {
	var tree = {
		...
		createNode: function(p_text,p_expanded, p_icon, p_parentNode,p_tag,p_contextmenu,p_dblclickCallback) {
			v_tree = this;
			node = {
				...
				tag : p_tag,
				contextMenu: p_contextmenu,
                dblclickCallback: p_dblclickCallback,
				elementLi: null,
				...
				createChildNode: function(p_text,p_expanded,p_icon,p_tag,p_contextmenu,p_dblclickCallback) { return v_tree.createNode(p_text,p_expanded,p_icon,this,p_tag,p_contextmenu,p_dblclickCallback); }
			}
        ...
        ///// Set node active
        // node: the node to set active;
        setActive: function(node) {
            if (this.activeNode) this.activeNode.icon = 'images/folder.png';
            if (node) node.icon = 'images/folder_active.png';
      ...
      this.adjustLines(document.getElementById(this.name),true);
		...
		drawNode: function(p_ulElement,p_node) {
            ...
            v_span.ondblclick = function() {
                if (p_node.dblclickCallback!=null)
                    p_node.dblclickCallback(p_node);
			};
			...
			if (p_event.button==2) {
				...
				if (p_node.contextMenu!=undefined) {
					...
					if (this.contextMenuDiv==null) {
						v_div = createSimpleElement('ul','ul_cm','treeMenu');
                    ...
                    var nodeActive = (this.activeNode === p_node);
					for (var i=0; i<v_menu.elements.length; i++) (function(i)
                    {
                        var item = v_menu.elements[i];
                        if (item.seperator === true)
                        {
                            var v_li = createSimpleElement('li',null,'separator');
                            v_div.appendChild(v_li);
                            return;
                        }
                        // Maybe need to hide this item?
                        if (nodeActive && item.hideWhenActive) return;
                        if (!nodeActive && item.hideWhenNotActive) return;

						var v_li = createSimpleElement('li',null,item.title?'title':null);

						var v_span = createSimpleElement('span',null,null);
                        if (item.title === true)
                        {
                            item.text = p_node.text;
                        }
                        else
                        {
                            v_span.onclick = function () {  item.action(p_node) };
                        }

						var v_a = createSimpleElement('a',null,null);
						var v_ul = createSimpleElement('ul',null,'sub-menu');

						v_a.appendChild(document.createTextNode(item.text));

						v_li.appendChild(v_span);

						if (item.icon!=undefined) {
							var v_img = createImgElement('null','null',item.icon);
						...
						if (item.submenu!=undefined) {
							...
							v_span_more.appendChild(createImgElement(null,'menu_img','images/subright.png'));
							v_li.appendChild(v_span_more);
							v_tree.contextMenuLi(item.submenu,v_ul,p_node);
						}
